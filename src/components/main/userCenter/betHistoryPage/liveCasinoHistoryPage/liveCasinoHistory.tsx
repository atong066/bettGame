...
function LiveCasinoHistory(props: any) {
  const { t, i18n } = useTranslation(["home", "main"]);
  const colorP = ChangeColorPallte()
  const [pageCount, setPageCount] = useState(0)
  const [currentPage, setCurrentPage] = useState(1)
  const [liveCasinoHistory, setLiceCasinoHistory] = useState([])
  const [isData, setIsData] = useState(false)
  const [setLoader, setOpenLoader] = useState(true);
  const getData = () => {
    setOpenLoader(true);
    try {
      axios.post('/userCenter/third/liveRecord.do', {
        ...
        if (result.data.rows != '') {
          ...
          setLiceCasinoHistory(result.data.rows)
          setIsData(true)
          setOpenLoader(false);
        } else {
          setIsData(false)
  ...
  return (
    <>
      ...
      <Table>
        ...
        <TableBody>
          {isData == true && liveCasinoHistory?.map((value: any, index: any) =>
            <TableRow key={index}>
              ...
              <TableCell>{value.tableCode}</TableCell>
...
export default LiveCasinoHistory;
