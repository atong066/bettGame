...
function FishingHistory(props: any) {
  const { t, i18n } = useTranslation(["home", "main"]);
  const colorP = ChangeColorPallte()
  const [pageCount, setPageCount] = useState(0)
  const [currentPage, setCurrentPage] = useState(1)
  const [FishingHistory, setFishingHistory] = useState([])
  ...
  const getData = () => {
    try {
      setOpenLoader(true);
      axios.post('/userCenter/third/fishRecord.do', {
        ...
        if (result.data.rows != '') {
          ...
          setFishingHistory(result.data.rows)
          setIsData(true)
          setOpenLoader(false);
  ...
  return (
    <>
      ...
      <Table>
        ...
        <TableBody>
          {isData == true && FishingHistory?.map((value: any, index: any) =>
            <TableRow key={index}>
              ...
              <TableCell>{value.sceneId}</TableCell>
